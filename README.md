...
outputs/
└── <회사>_<직무>_<타임스탬프>/
    ├── analysis_results.json      # 전체 분석 결과(내부용)
    ├── summary.json               # 요약 정보
    ├── cover_letter.txt           # 자기소개서 합본(문항별 답변 포함)
    ├── cover_letters/             # 문항별 개별 텍스트
    └── guides/                    # question/experience/writing 가이드 파일들
```
...
### **문서 유형**
- `resume`: 이력서 작성(ResumeWriter) → 자기소개서(옵션, CoverLetterWriter) → 품질
- `cover_letter`: 자기소개서만 작성(CoverLetterWriter) → 품질
